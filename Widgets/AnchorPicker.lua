...
-- ============================================================
-- AnchorPicker — 3×3 anchor grid with X/Y offset inputs
...
-- 3×3 grid order: row-major, top-left to bottom-right
local ANCHOR_GRID = {
...
function Widgets.CreateAnchorPicker(parent, width)
    ...
    for i, point in ipairs(ANCHOR_GRID) do
    ...
    function picker:_SelectPoint(point)
        ...
        -- Select new
        self._point = point
        local next = self._buttons[point]
        if next then
            next._selected = true
            ApplyButtonSelected(next, true)
        end
