...
F.Settings.RegisterPanel({
	...
	create     = function(parent)
		...
		local filterDDW  = fieldW - nameBoxW - C.Spacing.normal
		...
		local filterDD = Widgets.CreateDropdown(formFrame, filterDDW)
		filterDD:SetItems(FILTER_MODE_ITEMS)
		filterDD:SetValue('all')
		filterDD:SetOnSelect(function(v) selectedFilter = v end)
		filterDD:ClearAllPoints()
		Widgets.SetPoint(filterDD, 'TOPLEFT', nameBox, 'TOPRIGHT', C.Spacing.normal, -math.floor((FORM_ROW_H - DROPDOWN_H) / 2))
		...
		local function onResize(newW, newH)
			...
			local newFilterDDW  = newFieldW - newNameBoxW - C.Spacing.normal
			nameBox:SetWidth(newNameBoxW)
			filterDD:SetWidth(newFilterDDW)
