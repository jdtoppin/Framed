...
local function get(key)
	...
	return F.Config and F.Config:Get('presets.' .. presetName .. '.auras.' .. unitType .. '.externals.' .. key)
end
...
local function set(key, value)
	...
	local unitType   = F.Settings.GetEditingUnitType and F.Settings.GetEditingUnitType() or 'party'
	if(F.Config) then
		F.Config:Set('presets.' .. presetName .. '.auras.' .. unitType .. '.externals.' .. key, value)
	end
	if(F.PresetManager) then F.PresetManager.MarkCustomized(presetName) end
	if(key ~= 'enabled' and F.EventBus) then
		F.EventBus:Fire('CONFIG_CHANGED', 'presets.' .. presetName .. '.auras.' .. unitType .. '.externals')
	end
end
