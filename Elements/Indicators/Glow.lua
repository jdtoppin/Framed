...
--- @return table glow
function Framed.Indicators.Glow.Create(parent, config)
    config = config or {}
    ...
    if not LCG and Framed.Indicators.Border then
        fallbackBorder = Framed.Indicators.Border.Create(parent)
    end
    ...
    for k, v in pairs(GlowMethods) do
        glow[k] = v
    end
