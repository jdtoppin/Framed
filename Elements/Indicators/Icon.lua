...
function IconMethods:SetSpell(spellID, iconTexture, duration, expirationTime, stacks, dispelType)
    ...
    -- Cooldown swipe
    if self._config.showCooldown and self.cooldown then
...
--- @return table icon
function Framed.Indicators.Icon.Create(parent, size, config)
    config = config or {}
    ...
    -- Apply methods
    for k, v in pairs(IconMethods) do
        icon[k] = v
    end
