...
function IconsMethods:SetIcons(auraList)
    ...
    for i = 1, count do
        ...
        if not self._pool[i] then
            self._pool[i] = Framed.Indicators.Icon.Create(container, cfg.iconSize, {
...
--- }
--- @return table icons
function Framed.Indicators.Icons.Create(parent, config)
    config = config or {}
    ...
    -- Apply methods
    for k, v in pairs(IconsMethods) do
        icons[k] = v
    end
